...
@with_kw struct MTSOptions
    ...
    function MTSOptions(args...; kwargs...)
        @assert (length(args) > 0 && args[1] in OPTIMIZATION_METHODS) || kwargs[:method] in OPTIMIZATION_METHODS
        new(args...; kwargs...)
    end
end
...
function _localsearch2(workspace, k)
    ...
    for i in 1:model.n_dim
        ...
        update_xk = true
        D = [rand([-1, 1]) for _ in 1:model.n_dim]
        r = [rand([0, 1, 2, 3]) for _ in 1:model.n_dim]
        # Value after update
        _xk = copy(x[k])
...
const OPTIMIZATION_METHODS = [localsearch1, localsearch2, localsearch3, MTS]
